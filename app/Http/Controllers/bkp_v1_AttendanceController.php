...
class bkp_v1_AttendanceController extends Controller
{
    /*
    ...
    */
    ...
    {
        return $user = User::where(['id'=>$request->id])
                    ->with('employee')
    ...
    */
    function consolidatedAttendanceSheets(Request $request)
    {
		...
		//exit;
		//$employees[$key]->login_user_designation = $designation_login_user;
...
$i++;
		...
		if($request->submit == 'export excel sheet'){
            $employee_collection = $employees_po;
        ...
        if($request->submit == 'saral excel sheet'){
            $employee_collection = collect( $employees_po);
			/* echo"<pre>";
			print_r($employee_collection);
			exit; */
            $set_index_collection = $employee_collection->map(function($item, $key){
                $item->user_id = $key+1;
                $item->unpaid_leaves = $item->unpaid_leaves + $item->absent_days;

				unset($item->designation_id);
				unset($item->state_id);

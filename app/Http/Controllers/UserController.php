...
//	          return $user->email;
    ...
    */
...

    {
		...
		$url = 'https://www.google.com/recaptcha/api/siteverify?secret='.$data['secret'].'&response='.$data['response'].'&remoteip='.$data['remoteip'];
        ...
        if ($resultJson->success != true || $resultJson->score < 0.3) {
            return back()->withErrors(['captcha' => 'ReCaptcha error, please try again.']);
        }
    ...
    */
    ...
    function dashboard(Request $request)
    {
        ...
        $user_filled = Employee::where(['user_id'=>$user_info->id])->first();
...
//        return $data['designation'];
    ...
    */
    ...
    function list_all(){
        ...
        return view('employees.list_all')->with(['departments'=>$departments,'projects'=>$projects, 'data_emp'=>$employees_po]);
...
//$i++;
    ...
    */
...

    {
        ...
        $data['departments'] = Department::where(['isactive'=>1])->select('id','name')->orderBy('name')->get();

        $data['locations'] = Location::where(['isactive'=>1])->select('id','name')->orderBy('name')->get();
...
//        }
    ...
    */
    ...
    public function editLocation(Request $request){
        ...
        elseif($request->designation_id==2){
            $validator = Validator::make($request->all(), [
                'stateId' => 'required',
            ]);
        }
        ...
        if($request->designation_id==2){
            //SPO manage state
            EmployeeProfile::where('user_id', $request->employeeId)
                ->update([
                    'state_id'=>$request->stateId
                ]);
        }

        return redirect()->back()->with('documentSuccess',"Details saved successfully.");
  ...
  */
    ...
    function profile_detail_form()
    {
        ...
        $details = [];
        return view('employees.profile_detail_form')->with(['data'=>$data]);
    }
    ...
    function saveuserdata(Request $request){
        ...
        foreach($users_data as $data){
            ...
            if($emp_update){
                ...
                echo $user_data->employee_code. " with user id ".$user_id." father name and ";
            }


            $emp_account_exist = EmployeeAccount::where(['user_id'=>$user_id])->first();
            ...
            if($emp_account_exist){
                ...
                if( $emp_account_update){
                    ...
                    echo $user_data->employee_code. " with user id ".$user_id." account has been updated"."<br/>";
                }
            }else{
                ...
                if( $emp_account_create){
                    ...
                    echo $user_data->employee_code. " with user id ".$user_id." account has been created"."<br/>";
                }
